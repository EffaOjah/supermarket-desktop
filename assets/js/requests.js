...
async function tryFetchProducts() {
    ...
    const data = await window.electronAPI.syncProducts(lastSyncedDate[0].lastSyncedDate);
    ...
    for (const product of data.products) {
        ...
        const checkStock = await window.sqlite.storeManager('checkTheStock', product.product_name);
        console.log(checkStock);

        if (checkStock.length > 0) {
            console.log('Product already exists');
            const updateProduct = await window.sqlite.storeManager('updateProduct', checkStock[0].product_id, product.product_name, product.wholesale_price, product.retail_price, product.category);
            console.log(updateProduct);
        } else {
            const insertProducts = await window.sqlite.storeManager('addProduct', product.product_id, product.product_name, product.wholesale_price, product.retail_price, product.supplier_id, product.category);
